...
<?php
	...
	if(isset($_FILES['file'])){
		...
		if(in_array($file_ext, $allowed)){
			if($file_error === 0){
				...
				if(move_uploaded_file($file_tmp, "../drywest/dashboard/Admin/".$file_destination)){
					...
					echo "<script>
						...
						window.location= '../drywest/dashboard/examples/newfile.php';
					</script>";					
	...
	function buscar($texto, $palabra, $inicio, $final, $i, $pagina, $lid, $dbconn)
    {
        ...
        if($substring !== false){
			...
			$query = "INSERT INTO contenido(lid, articulo_inicio, contenido, pagina) VALUES ('$lid', '$i', '$substring', '$pagina')";
			$result = pg_query($dbconn, $query) or die('Query failed: ' . pg_last_error());
